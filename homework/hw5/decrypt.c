...
int in_dict(char *word)
{
    ...
    // yayyy binary search for effiency
    int left = 0, right = word_count - 1;
    
    while (left < right) {
        int middleIndex = (left + right) / 2;
        if (strcmp(word, words[middleIndex]) > 0) {
            left = middleIndex + 1;
        } else if (strcmp(word, words[middleIndex]) < 0) {
            right = middleIndex - 1;
        } else {
            return 1;
        }
...
void decryption(unsigned char key, unsigned char shift, const int *encrypted, int len, char *decrypted)
{
    ...
    for (int i = 0; i < len; i++) { 
        // int firstShift = encrypted[i] ^ key;
        // int secondShift = firstShift >> shift;
        // decrypted[i] = (char) secondShift;
...
int message_score(const char *msg)
{
    ...
    char * tokenized = strtok(copyMsg, " ");

    while (tokenized != NULL) {
        if (in_dict(tokenized)) score++;
...
int read_encrypted(char *filename, int *encrypted)
{
    ...
    int count = 0;
    while (fread(&encrypted[count], sizeof(int), 1, file) == 1) {
        count++;
    }
