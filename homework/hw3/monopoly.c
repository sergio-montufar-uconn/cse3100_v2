...
{
	int id;  // this determines order of turn (in increasing order) from a range of 0 to m - 1
	int loc; // this is to determine which property the player is currently on, from a range of 0 to n - 1
	long balance; // balance a player has
} TPlayer;

//There will be n properties in an array
typedef struct Property
{
	int id;        // range of 0 to n - 1
	int owner_id;  // range of 0 to m - 1
	int rent;      // rent of property
} TProperty;
...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		int steps = rand() % 6 + 1 + rand() % 6 + 1; // this is the functionality of the dice 
		//fill in the code below
		
		// check if a player already owns the property they're standing on
		p[i].loc += steps;
		int playerLoc = p[i].loc;

		if (prop[playerLoc].owner_id == -1) { // -1 means no one owns this property
			prop[playerLoc].owner_id = p[i].id; 

		} else if (prop[playerLoc].owner_id != p[i].id) { // if current player does NOT own the property
			int idToFindPlayerLoc = prop[playerLoc].owner_id;

			if (!transaction(&p[i], &p[idToFindPlayerLoc], prop[playerLoc].rent)) {
				return 0; // game ends since player goes bankrupt
			}
		}

		// check if player passes the last property
		if (p[i].loc >= n) { 
			p[i].balance += n;
			p[i].loc -= n;
		}
