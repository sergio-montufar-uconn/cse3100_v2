...
{
	while (head != NULL) {
		int head_x = head->host.x;
		int head_y = head->host.y;

		if (head_x == host.x && head_y == host.y)
			return 1;
...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
	...
	for(int i = 0; i < m; i++)
	{
		...
		switch(r)
		{
			case 0:
			//up
				if (hosts[i].y == k) hosts[i].y = -1 * k;
				else hosts[i].y += 1;
				break;

			case 1: 
			//right
				if (hosts[i].x == k) hosts[i].x = -1 * k;
				else hosts[i].x += 1;
				break;

			case 2: 
			//down
				if (hosts[i].y == -1 * k) hosts[i].y = k;
				else hosts[i].y -= 1;
				break;

			case 3: 
			//left
				if (hosts[i].x == -1 * k) hosts[i].x = k;
			     else hosts[i].x -= 1;
				break;
		}
