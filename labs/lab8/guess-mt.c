...
void * thread_c(void * arg_in)
{
    ...
    // send the result and, if the guess is correct, the final message to thread_p
        ...
        //unlock at the end then break if right
        arg->status = S_RESULT;
        pthread_cond_signal(&arg->cond_result);
        pthread_mutex_unlock(&arg->mutex);
...
int main(int argc, char *argv[])
{
    ...
    //Initalize the threads, guess & result
    pthread_mutex_init(&arg.mutex, NULL);
    pthread_cond_init(&arg.cond_result, NULL);
    pthread_cond_init(&arg.cond_guess, NULL);
    
    //Very creative names obviously 
    pthread_t thread_1, thread_2;

    //create the threads 
    pthread_create(&thread_1, NULL, thread_c, &arg);
    pthread_create(&thread_2, NULL, thread_p, &arg);
