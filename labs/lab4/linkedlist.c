...
node *delete_node(node *head, int v) {
  // TODO
  node * tempHead = head; // temp head if head is the one to be deleted
  if (head->v == v) {
    tempHead = head->next;
    free(head);
    return tempHead;
  ...
  while (tempHead != NULL && tempHead->v != v) {
      prevNode = tempHead;
      tempHead = tempHead->next;
  }
...
node *reverse_list(node *head) {
  ...
  node *pointerPrev = NULL, *pointerCurr = head, *pointerNext = head;
  while (pointerCurr != NULL) {
    pointerNext = pointerNext->next; // change next pointer to next's next pointer
    pointerCurr->next = pointerPrev;  // change current's next pointer to previous one

    pointerPrev = pointerCurr;  // change previous pointer to current pointer
    pointerCurr = pointerNext;  // change current pointer to next's pointer
