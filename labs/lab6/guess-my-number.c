...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    close(fdp[PIPEFD_WRITE]); // close read fd for parent pipe
    close(fdc[PIPEFD_READ]); // close read fd for child pipe

    int max = gmn_get_max();

    write(fdc[PIPEFD_WRITE], &max, sizeof(int)); // do error check if necessary

    int check, guessFromParent;

    do {
        read(fdp[PIPEFD_READ], &guessFromParent, sizeof(int));
        check = gmn_check(&gmn, guessFromParent);
        write(fdc[PIPEFD_WRITE], &check, sizeof(int));
        // printf("%d\n", guessFromParent);

    } while (check != 0);
    char * message = gmn_get_message(&gmn);
    write(fdc[PIPEFD_WRITE], message, strlen(message) + 1);
...
int main(int argc, char *argv[])
{
    ...
    // char message[MSG_BUF_SIZE];
    // read(fdc[PIPEFD_READ], &message, strlen(message) + 1);
    // fputs(message, stdout);

    char c = "";
    while (c != '\n') {
        read(fdc[PIPEFD_READ], &c, sizeof(char));
